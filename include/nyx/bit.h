...
int nyx_get_bit(const void *bits, uint64_t bit, int *value);
...
void nyx_copy_bits_unsafe(void *restrict dst_bits, uint64_t dst_offset, const void *src, uint64_t len);
int nyx_copy_bits(void *restrict dst_bits, uint64_t dst_len, uint64_t dst_offset, const void *src, uint64_t len);
